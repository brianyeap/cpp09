...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/04/17 19:38:54 by bryeap            #+#    #+#             */
/*   Updated: 2026/04/19 13:57:23 by bryeap           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
sortVector()
{
    if (_vec.size() <= 1) // alr sorted
        return;

    bool hasStraggler = false;
    int straggler = 0;

    if (_vec.size() % 2 != 0)      // if odd, save the last num as straggler and remove it from vector
    {
        hasStraggler = true;
        straggler = _vec.back();
        _vec.pop_back();
    ...
    for (size_t i = 0; i < _vec.size(); i += 2)
    {
        ...
        int second = _vec[i + 1];
        if (first < second)
            std::swap(first, second); // making usre the first num is always bigger than the second num 
        pairs.push_back(std::make_pair(first, second));   
    }

    std::vector<int> mainChain;
    for (size_t i = 0; i < pairs.size(); i++)   // push the bigger num of each pair to main chain
    {
        mainChain.push_back(pairs[i].first);
    }

    if (mainChain.size() > 1) // resursively process the main chain until it is sorted
    {
        _vec = mainChain;
        sortVector();
        mainChain = _vec;
    ...
    {
        pendChain.push_back(pairs[i].second);  // push the smaller num of each pair to pend chain
    }

    if (!pendChain.empty())
    {
        std::vector<int>::iterator pos = std::lower_bound(result.begin(), result.end(), pendChain[0]); // get the position to insert pend chain element
...
void PmergeMe::sort()
{
    ...
    double vecTime = static_cast<double>(vecEnd - vecStart) / CLOCKS_PER_SEC * 1000000; // get it in tick and convert to microsecond
    ...
    printSeq(_vec, "After (Vector)");
    printSeq(_deq, "After (Deque)");
